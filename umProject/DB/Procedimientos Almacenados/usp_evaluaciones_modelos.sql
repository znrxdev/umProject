                        ...
                        BEGIN
                            BEGIN TRY
                                SELECT 
                                    ...
                                    em.Id_Materia,
                                    em.Id_Tipo_Evaluacion,
                                    em.Codigo_Modelo,
                                    em.Nombre_Evaluacion,
                                    em.Concepto,
                                    em.Version_Configuracion,
                                    em.Rubrica_Detalle,
                                    em.Activo,
                                    em.Fecha_Creacion,
                                ...
                                LEFT JOIN cls_materias(NOLOCK) m ON em.Id_Materia = m.Id_Materia
                                LEFT JOIN cls_catalogos(NOLOCK) c ON em.Id_Tipo_Evaluacion = c.Id_Catalogo
                                WHERE em.Id_Evaluacion_Modelo = @Id_Evaluacion_Modelo;
            ...
            ELSE IF(@Id_Tipo_Transaccion = 123)
                BEGIN
                    IF ISNULL(@Id_Materia, 0) = 0
                        BEGIN
                            SET @o_Num = -1;
                            SET @o_Msg = '¡No ha seleccionado la materia para listar sus modelos de evaluación!';
                        END
                    ELSE
                        BEGIN
                            BEGIN TRY
                                SELECT 
                                    em.Id_Evaluacion_Modelo, em.Id_Materia, em.Id_Tipo_Evaluacion, em.Codigo_Modelo,
                                    em.Nombre_Evaluacion, em.Concepto,
                                    em.Version_Configuracion, em.Rubrica_Detalle, em.Activo,
                                ...
                                LEFT JOIN cls_materias(NOLOCK) m ON em.Id_Materia = m.Id_Materia
                                LEFT JOIN cls_catalogos(NOLOCK) c ON em.Id_Tipo_Evaluacion = c.Id_Catalogo
                                WHERE em.Id_Materia = @Id_Materia AND em.Activo = 1
                                ORDER BY em.Codigo_Modelo;

                                SET @o_Num = 0;
                                SET @o_Msg = '¡Evaluaciones modelos filtradas por materia!';
                            END TRY
                            BEGIN CATCH
                                SET @o_Num = -1;
                                SET @o_Msg = '¡Error interno del servidor!';
            ...
            ELSE IF(@Id_Tipo_Transaccion = 120)
                BEGIN
                    ...
                    ELSE IF EXISTS(SELECT 1 FROM cls_evaluaciones_modelos(NOLOCK) 
                        ...
                        BEGIN
                            SET @o_Num = -1;
                            SET @o_Msg = '¡Ya existe un modelo de evaluación con ese código y versión para esa materia!';
                        END
                    ELSE
                        BEGIN
                            ...
                            BEGIN TRY
                                INSERT INTO cls_evaluaciones_modelos(
                                    Id_Materia, Id_Tipo_Evaluacion, Codigo_Modelo, Nombre_Evaluacion,
                                    Concepto, Version_Configuracion,
                                    Rubrica_Detalle, Activo,
                                    Fecha_Creacion, Fecha_Modificacion, Id_Creador, Id_Modificador,
                                    Id_Transaccion,
                                    Calificacion_Maxima
                                ) VALUES (
                                    @Id_Materia, @Id_Tipo_Evaluacion, @Codigo_Modelo, @Nombre_Evaluacion,
                                    @Concepto, ISNULL(@Version_Configuracion, 1),
                                    @Rubrica_Detalle, ISNULL(@Activo, 1),
                                    @Fecha_Creacion, @Fecha_Modificacion, @Id_Creador, @Id_Modificador,
                                    @Id_Transaccion,
                                    100
                                );
                ...
                BEGIN
                    ...
                    ELSE IF NOT EXISTS(SELECT 1 FROM cls_evaluaciones_modelos(NOLOCK) WHERE Id_Evaluacion_Modelo = @Id_Evaluacion_Modelo)
                        BEGIN
                            ...
                            SET @o_Msg = '¡La evaluación modelo no existe!';
                        END
                    ELSE IF ((@Id_Materia IS NOT NULL OR @Codigo_Modelo IS NOT NULL OR @Version_Configuracion IS NOT NULL) AND
                             EXISTS(SELECT 1 FROM cls_evaluaciones_modelos(NOLOCK) 
                                    WHERE Id_Materia = COALESCE(@Id_Materia, (SELECT Id_Materia FROM cls_evaluaciones_modelos WHERE Id_Evaluacion_Modelo = @Id_Evaluacion_Modelo))
                        ...
                        BEGIN
                            SET @o_Num = -1;
                            SET @o_Msg = '¡Ya existe otro modelo de evaluación con esa combinación de materia, código y versión!';
                        END
                    ELSE
                        BEGIN
