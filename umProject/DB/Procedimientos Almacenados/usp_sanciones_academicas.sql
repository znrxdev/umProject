...
BEGIN
    ...
    IF(@Permiso = 1)
        BEGIN
            /* LISTAR TODAS LAS SANCIONES ACADÉMICAS */
            IF(@Id_Tipo_Transaccion = 183)
                BEGIN
                    BEGIN TRY
                        SELECT 
                            sa.Id_Sancion,
                            ...
                            sa.Documento_Resolucion,
                            sa.Id_Usuario_Resolucion,
                            sa.Fecha_Resolucion,
                            sa.Id_Sancion_Origen,
                            sa.Codigo_Control,
                            sa.Fecha_Creacion,
                            sa.Fecha_Modificacion,
                            sa.Id_Creador,
                            sa.Id_Modificador,
                            sa.Id_Transaccion,
                            ...
                            ts.Nombre_Catalogo AS Nombre_Tipo_Sancion,
                            tf.Nombre_Catalogo AS Nombre_Tipo_Falta,
                            sv.Nombre_Catalogo AS Nombre_Severidad,
                            est.Nombre_Estado AS Nombre_Estado,
                            ur.Usuario AS Usuario_Resolucion,
                            pr.Primer_Nombre + ' ' + ISNULL(pr.Segundo_Nombre + ' ', '') + pr.Primer_Apellido + ' ' + ISNULL(pr.Segundo_Apellido, '') AS Nombre_Usuario_Resolucion
                        ...
                        LEFT JOIN cls_catalogos sv (NOLOCK) ON sa.Id_Severidad = sv.Id_Catalogo
                        LEFT JOIN cls_estados est (NOLOCK) ON sa.Id_Estado = est.Id_Estado
                        LEFT JOIN tbl_usuarios ur (NOLOCK) ON sa.Id_Usuario_Resolucion = ur.Id_Usuario
                        LEFT JOIN tbl_personas pr (NOLOCK) ON ur.Id_Persona = pr.Id_Persona
                        ORDER BY sa.Fecha_Registro DESC;

                        SET @o_Num = 0;
                        SET @o_Msg = '¡Sanciones académicas listadas exitosamente!';
                    END TRY
                    BEGIN CATCH
                        SET @o_Num = -1;
            ...
            ELSE IF(@Id_Tipo_Transaccion = 88)
                BEGIN
                    ...
                    ELSE
                        BEGIN
                            BEGIN TRY
                                ...
                                LEFT JOIN tbl_usuarios ur (NOLOCK) ON sa.Id_Usuario_Resolucion = ur.Id_Usuario
                                LEFT JOIN tbl_personas pr (NOLOCK) ON ur.Id_Persona = pr.Id_Persona
                                WHERE sa.Id_Estudiante = @Id_Estudiante
                                ORDER BY sa.Fecha_Registro DESC;
                    ...
                    ELSE
                        BEGIN
                            BEGIN TRY
                                ...
                                LEFT JOIN tbl_usuarios ur (NOLOCK) ON sa.Id_Usuario_Resolucion = ur.Id_Usuario
                                LEFT JOIN tbl_personas pr (NOLOCK) ON ur.Id_Persona = pr.Id_Persona
                                WHERE sa.Id_Estudiante = @Id_Sesion
                                ORDER BY sa.Fecha_Registro DESC;
