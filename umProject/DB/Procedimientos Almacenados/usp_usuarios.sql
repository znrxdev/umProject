...
BEGIN
	...
	/* ACTUALIZAR ULTIMA SESION - No requiere validación de permisos */
	IF(ISNULL(@Id_Usuario, 0) <> 0 AND ISNULL(@EstadoLogin, 0) <> 0)
		BEGIN
			IF NOT EXISTS(SELECT 1 FROM tbl_usuarios(NOLOCK) WHERE Id_Usuario = @Id_Usuario)
				BEGIN
					SET @o_Num = -1;
			...
			ELSE
				BEGIN
					BEGIN TRY
						...
						SET @o_Num = 0;
						SET @o_Msg = '¡Última sesión actualizada correctamente!';
					END TRY
					BEGIN CATCH
						SET @o_Num = -1;
						SET @o_Msg = '¡Error al actualizar última sesión!';
