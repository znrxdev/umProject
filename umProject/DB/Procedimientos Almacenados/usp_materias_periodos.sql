            ...
            ELSE IF(@Id_Tipo_Transaccion = 96)
                BEGIN
                    BEGIN TRY
                        ...
                        WHERE mp.Activo = 1
                        AND pa.Id_Estado IN (3, 4) -- Solo períodos en estado PENDIENTE (3) o EN REVISION (4)
            ...
            ELSE IF(@Id_Tipo_Transaccion = 91)
                BEGIN
                    ...
                    ELSE IF (@Porcentaje_Asistencia_Minima IS NOT NULL AND (@Porcentaje_Asistencia_Minima < 0 OR @Porcentaje_Asistencia_Minima > 100))
                        ...
                        END
                    ELSE IF (@Porcentaje_Asistencia_Minima IS NOT NULL AND @Porcentaje_Asistencia_Minima < 50.00)
                        BEGIN
                            SET @o_Num = -1;
                            SET @o_Msg = '¡El porcentaje de asistencia mínima debe ser al menos 50%!';
                        END
                    ELSE IF EXISTS(SELECT 1 FROM cls_materias_periodos(NOLOCK) WHERE Id_Materia = @Id_Materia AND Id_Periodo_Academico = @Id_Periodo_Academico)
                        BEGIN
                            SET @o_Num = -1;
                            SET @o_Msg = '¡Esta materia ya está asignada a este período académico!';
                        END
                    ELSE
                        BEGIN
                            -- Generar Codigo_Plan automáticamente como "PLAN-AÑO ACTUAL" si viene vacío
                            IF ISNULL(@Codigo_Plan, '') = ''
                                BEGIN
                                    DECLARE @AnioActual INT = YEAR(GETDATE());
                                    SET @Codigo_Plan = CONCAT('PLAN-', @AnioActual);
                                END
                            
                            -- Forzar Id_Jornada a NULL siempre
                            SET @Id_Jornada = NULL;
                            
                            -- Establecer Porcentaje_Asistencia_Minima por defecto en 75% si es NULL
                            IF @Porcentaje_Asistencia_Minima IS NULL
                                BEGIN
                                    SET @Porcentaje_Asistencia_Minima = 75.00;
                                END
                            
                            -- Validar que el porcentaje establecido sea >= 50%
                            IF @Porcentaje_Asistencia_Minima < 50.00
                                BEGIN
                                    SET @o_Num = -1;
                                    SET @o_Msg = '¡El porcentaje de asistencia mínima debe ser al menos 50%!';
                                    RETURN;
                            ...
                            BEGIN TRY
                                INSERT INTO cls_materias_periodos(
                                    ...
                                    @Id_Materia, @Id_Periodo_Academico, @Codigo_Plan, NULL,
