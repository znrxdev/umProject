...
(
    ...
    [Codigo_Programa] VARCHAR(30) NOT NULL CONSTRAINT DF_cls_becas_programas_Codigo DEFAULT(CONCAT('BP-', FORMAT(GETDATE(), 'yyyyMMddHHmmss'), '-', RIGHT(CONVERT(VARCHAR(8), ABS(CHECKSUM(NEWID()))), 4))),
    ...
    [Id_Estado_Programa] INT NOT NULL REFERENCES dbo.cls_estados(Id_Estado) DEFAULT(4),
    [Fecha_Creacion] DATETIME NOT NULL,
    [Fecha_Modificacion]  DATETIME NOT NULL,
...
(
    ...
    [Id_Estado] INT NOT NULL REFERENCES dbo.cls_estados(Id_Estado) DEFAULT(4), -- Por defecto EN REVISION
    [Fecha_Creacion] DATETIME NOT NULL,
    [Fecha_Modificacion] DATETIME NOT NULL,
...
(
    ...
    [Activo] BIT NOT NULL,
    [RowVersion] ROWVERSION NOT NULL,
    CONSTRAINT UQ_cls_evaluaciones_modelos UNIQUE([Id_Materia], [Codigo_Modelo], [Version_Configuracion]),
...
(
    ...
    [Calificacion_Maxima] DECIMAL(6,2) NOT NULL DEFAULT(0),
    [Id_Periodo] INT NOT NULL REFERENCES dbo.tbl_periodos_academicos(Id_Periodo),
...
-- Índice ajustado (sin columnas eliminadas)
CREATE NONCLUSTERED INDEX IX_tbl_solicitudes_becas_ProgramaEstadoEstudiante
ON dbo.tbl_solicitudes_becas(Id_Beca_Programa, Id_Estado, Id_Estudiante);
...
-- Índice IX_tbl_inscripciones_SeccionEstado eliminado: Id_Seccion ya no existe en tbl_inscripciones
