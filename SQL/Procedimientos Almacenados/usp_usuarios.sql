	...
	IF(@Permiso = 1 OR @Id_Tipo_Transaccion = 19)
			...
			IF (@Id_Tipo_Transaccion = 19)
				BEGIN
					IF NOT EXISTS (SELECT 1 FROM tbl_usuarios U WITH (NOLOCK) WHERE U.Usuario = @Usuario)
						BEGIN
							SET @o_Num = 0;
							SET @o_Msg = 'Usuario y/o Contraseña incorrecta.';
						END
			...
			ELSE IF(@Id_Tipo_Transaccion = 20)
				BEGIN
					...
					ELSE IF ISNULL(CONVERT(VARCHAR(100),@Contrasena), '') = ''
						BEGIN
							SET @o_Num = -1;
							SET @o_Msg = '¡Debe digitar una contraseña!';
						END
					...
					ELSE
						BEGIN
							...
							BEGIN CATCH
								...
								ROLLBACK TRAN trx_AgregarVentaFlotante
								SET @o_Num = -1;
		...
		/* ACTUALIZAR USUARIOS */
			ELSE IF(@Id_Tipo_Transaccion = 21)
				BEGIN
					...
					ELSE IF ISNULL(CONVERT(VARCHAR(100),@Contrasena), '') = ''
						BEGIN
							SET @o_Num = -1;
							SET @o_Msg = '¡La contraseña no puede estar vacía!';
						END
					ELSE IF LEN(@Contrasena) < 8
						BEGIN
							SET @o_Num = -1;
							SET @o_Msg = '¡La contraseña debe ser mayor a 8 carácteres!';
						END
					ELSE
						BEGIN
							SET @iConcepto = IIF(@Contrasena IS NULL, 'ACTUALIZANDO USUARIO', 'CONTRASEÑA ACTUALIZADA')
							EXEC sp_transacciones
								@Modo = 'INS',
								@Id_Tipo_Transaccion = @Id_Tipo_Transaccion,
								@Concepto = @iConcepto,
			...
			ELSE
				BEGIN
					SET @o_Num = -1;
					SET @o_Msg = '¡Tipo transacción invalida!';
				END
		END
	ELSE
		BEGIN
